...
const loginTemplate = readFileSync(
  new URL("./views/login.html", import.meta.url),
  "utf-8",
);
  ...
  async (req, res) => {
    ...
    const renderError = (message: string) => {
      const html = loginTemplate
        ...
        .replaceAll("{{ERROR}}", `<div class="error">${message}</div>`);
    ...
    try {
      ...
      const { key: fireboardToken } = (await loginRes.json()) as {
        key: string;
      };
  ...
  requestContext
    .run({ reqId }, async () => {
      ...
      const tool: string | undefined =
        method === "tools/call" ? req.body?.params?.name : undefined;
      ...
      if (!token) {
        ...
        res.setHeader(
          "WWW-Authenticate",
          `Bearer realm="${issuerUrl.href}", resource_metadata="${issuerUrl.href}.well-known/oauth-protected-resource"`,
        );
