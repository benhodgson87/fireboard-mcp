...
describe('transformDriveLog', () => {
  ...
  it('maps degreetype 2 to F', () => {
    expect(transformDriveLog({ ...baseDriveLog, degreetype: 2 }).unit).toBe('F')
  })
...
describe('transformDeviceSummary', () => {
  ...
  it('includes last_drive when last_drivelog present', () => {
    const result = transformDeviceSummary({ ...baseDevice, last_drivelog: baseDriveLog })
...
describe('transformSessionSummary', () => {
  ...
  it('maps device_ids to device_uuids', () => {
    ...
    expect(transformSessionSummary(session).device_uuids).toEqual(['uuid-1', 'uuid-2'])
  })
})
...
describe('transformSessionDetail', () => {
  ...
  it('flattens channels from all devices', () => {
    const result = transformSessionDetail(baseSession)
    expect(result.channels).toEqual([{ label: 'Ambient', device_uuid: 'device-1' }])
  })
})
...
describe('transformSessionChart', () => {
  ...
  it('includes notes from session detail', () => {
    const session = {
      ...baseSession,
      notes: [{ note_time: '2026-06-13T10:00:00Z', note_text: 'Wrapped', channel: null, device: 'device-1' }],
    }
